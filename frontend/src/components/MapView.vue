...
export default {
  props: {
    // New prop to receive a polygon (as GeoJSON) from the search component
    predioPolygon: {
      type: Object,
      default: null
    }
  ...
  data() {
    return {
      ...
      tileLayer: null,
      uploadedLayer: null,
      predioLayer: null, // Layer for polygon coming from search results
      isHidden: false,
      loading: false,
      cursorLat: 0,
      cursorLng: 0,
  ...
  watch: {
    // When the predioPolygon prop changes, update the map layer accordingly.
    predioPolygon(newVal) {
      if (newVal && this.map) {
        // Remove previous search layer if it exists.
        if (this.predioLayer) {
          this.map.removeLayer(this.predioLayer);
        }
        // Add the new polygon layer (styled in red).
        this.predioLayer = L.geoJSON(newVal, {
          style: { color: "red", weight: 2 }
        }).addTo(this.map);
        // Fit the map bounds to the new polygon.
        this.map.fitBounds(this.predioLayer.getBounds());
      }
    }
  },
  methods: {
